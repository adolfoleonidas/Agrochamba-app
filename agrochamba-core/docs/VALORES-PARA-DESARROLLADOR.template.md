...
1. **En `wp-config.php` de WordPress:**
   ```php
   define('JWT_AUTH_SECRET_KEY', 'AQUI_ESTA_TU_CLAVE_SECRETA');
   ```
...
- Pueden usar el valor real si necesitan compatibilidad con sistema antiguo
...
4. O revisa `wp-config.php`:
   ```php
   define('JWT_AUTH_SECRET_KEY', 'AQUI_ESTA_LA_CLAVE');
   ```
...
```env
WORDPRESS_URL=https://agrochamba.com
WORDPRESS_JWT_SECRET=[CONSULTA_TU_WP_CONFIG_PHP]
